...
from utils import days_ago_date, process_fetched, get_date, date_to_timestamp, process_text, format_recording
...
# TODO CONVERT STRING STATES TO ENUM
"""
    POSSIBLE STATES:
    - IDLE
    - AUD_INP_TP - audio processing (getting topic)
    - AUD_INP_LN - audio processing (getting language input)
    - AUD_PR - audio processing (downloading, converting, storing)
...
async def process_start_command(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
...
async def process_get_all(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        USER_DATA[msg.from_user.id]['state'] = 'GET_ALL'
...
async def process_get_by_date(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        USER_DATA[msg.from_user.id]['state'] = 'GET_BY_DATE' if msg.text == 'by date' else 'GET_ALL_AF'
...
async def process_get_date_kb(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] in ['GET_BY_DATE', 'GET_ALL_AF']:
        ...
        recordings = process_fetched(
            ...
            if USER_DATA[msg.from_user.id]['state'] == 'GET_BY_DATE' else
...
async def process_get_between(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        USER_DATA[msg.from_user.id]['state'] = 'GET_ALL_BW'
...
async def process_get_date_input(msg: types.Message):
    ...
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        ...
        len_without_time = 10
        datestamp = msg.text
        if USER_DATA[msg.from_user.id]['state'] == 'GET_BY_DATE':
            is_exact = True
            ...
            for recording in recordings:
                await msg.reply(format_recording(recording=recording),
                                reply_markup=get_rec_kb, parse_mode=ParseMode.HTML)
            USER_DATA[msg.from_user.id]['state'] = 'IDLE'
        elif USER_DATA[msg.from_user.id]['state'] == 'GET_ALL_AF':
            if len(datestamp) == len_without_time:
                datestamp = f'{datestamp} 00:00:00'
...
async def process_get_between_input(msg: types.Message):
    ...
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        len_with_time = 39
        len_without_time = 21
        if USER_DATA[msg.from_user.id]['state'] == 'GET_ALL_BW':
            datestamps = msg.text
...
async def process_get_last_n_command(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        USER_DATA[msg.from_user.id]['state'] = 'GET_LAST_N'
...
async def process_get_number_input(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] == 'GET_LAST_N':
...
async def process_get_by_topic_command(msg: types.Message):
    if 'state' not in USER_DATA[msg.from_user.id].keys():
        USER_DATA[msg.from_user.id]['state'] = 'IDLE'
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        USER_DATA[msg.from_user.id]['state'] = 'GET_ALL_TP'
...
async def process_get_text_input(msg: types.Message):
    ...
    if USER_DATA[msg.from_user.id]['state'] == 'GET_ALL_TP':
        ...
        USER_DATA[msg.from_user.id]['state'] = 'AUD_PR'
...
async def process_voice_message(msg: types.Message):
    ...
    if USER_DATA[msg.from_user.id]['state'] not in ['AUD_INP_TP', 'AUD_INP_LN', 'AUD_PR']:
        ...
        USER_DATA[msg.from_user.id]['state'] = 'AUD_INP_TP'
